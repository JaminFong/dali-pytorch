...
class Block(nn.Module):
    ...
    def __init__(self, in_ch, block_ch, head_op, stack_ops, stride, use_se=False):
        super(Block, self).__init__()
        self.head_layer = OPS[head_op](in_ch, block_ch, stride, use_se=use_se,
                                        affine=True, track_running_stats=True)

        modules = []
        for stack_op in stack_ops:
            modules.append(OPS[stack_op](block_ch, block_ch, 1, use_se=use_se,
                                        affine=True, track_running_stats=True))
...
class MBV2_Net(nn.Module):
    def __init__(self, net_config, config=None):
        ...
        use_se = False
        if hasattr(config.optim, 'use_se'):
            use_se = config.optim.use_se
    ...
    def __init__(self, net_config, config=None):
        ...
        for config in self.net_config:
            self.blocks.append(Block(config[0][0], config[0][1], 
                        config[1], config[2], config[-1], use_se=False))
...
class PLAIN_Net(MBV2_Net):
    def __init__(self, *args, **kwargs):
        super(PLAIN_Net, self).__init__(*args, **kwargs)
